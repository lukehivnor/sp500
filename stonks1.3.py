...
target_return = 0.01  # Target return (e.g., 1%)
# Constraints
constraints = [
...
def negative_sharpe_ratio(weights, mean_returns, cov_matrix, risk_free_rate=0.036):
    portfolio_return = np.dot(weights, mean_returns)
    portfolio_risk = np.sqrt(np.dot(weights.T, np.dot(cov_matrix, weights)))
    return -(portfolio_return - risk_free_rate) / portfolio_risk  # Negate for maximization


# Perform optimization
result = minimize(negative_sharpe_ratio, initial_weights,args=(mean_returns, cov_matrix, 0.036),
...
optimal_sharpe = (optimal_return - 0.036) / optimal_risk
