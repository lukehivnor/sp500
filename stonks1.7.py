...
def getter():
    sp = pd.read_csv("D://tester//sp500_5_yr_edited.csv")
...
# Split the dataset
train_size = int(0.8 * len(df['Open']))
val_size = int(0.1 * len(df['Open']))
...
y_train = pd.Series(train['Open'].values).fillna(train['Open'].mean())  # 1D Series
...
y_val = pd.Series(validation['Open'].values).fillna(train['Open'].mean())  # 1D Series

X_test = pd.DataFrame(test['Date'].values, columns=['Close/Last']).fillna(train['Close/Last'].mean())  # 2D DataFrame
y_test = pd.Series(test['Open'].values).fillna(train['Open'].mean())  # 1D Series
...
params = {
    ...
    'min_child_weight': 3,           # Minimum instance weight in a child
    'subsample': 0.4,                # Fraction of data used for each tree
